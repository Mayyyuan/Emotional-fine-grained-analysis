...
tf.flags.DEFINE_string('predict_result', '../result/res0708', 'the predict result')
...
# tf.flags.DEFINE_string("model_name", "../runs/1531046501/checkpoints-950", "Name(Path) of model")
tf.flags.DEFINE_string("model_name", "../runs/1531074096/checkpoints-38050", "Name(Path) of model")
...
with sess1.as_default():
    with g1.as_default():
        ...
        feed_dict = {
            ...
            g1.get_tensor_by_name('keep_prob:0'): 1.0
        }
...
###################################get the end result########################################
        ...
        arr2 = test_predict.reshape((sentence_num, 300))
        # print(arr2[114])
        ...
        for i, line in enumerate(lines):
            ...
            for j, tag in enumerate(res):
                ...
                if(tag==1 and sen_flag==0):
                    ...
                    theme.append(line[j])
            o.write('theme:'+';'.join(theme) + '\tsentiment:'+ ';'.join(sentiment) + '\n')
            sentiment = []
            theme = []
        o.close()
print('predict is ok!')
